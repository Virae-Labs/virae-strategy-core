...
description: Evaluate, explain, and replay Virae prediction-market strategies locally with @viraeai/virae-strategy-core. Use when a user asks about Crypto Tail or Pre-M strategy logic, wants deterministic decisions or order intents from supplied snapshots, needs a local replay, or wants to compare local results with Virae Auto Trade. This skill never fetches market data, accesses wallets, signs transactions, or submits orders.
---
